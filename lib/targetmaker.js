...
function evaluateFilters(values, filters) {
    ...
    // True if
    //  - there is a value for every filter
    //  - that value is not 'noSuchInstance'
    //  - the value passes the filter test
    var result = _.all(filters, function (filter) {
        var value = _.find(values, function (val, oid) { return oid.indexOf(filter.oid) === 0; });
        return typeof value !== 'undefined' && value !== 'noSuchInstance' && evaluateFilter(value, filter.op, filter.rhs);
    });
...
function getPackage(conn, coll, callback) {
    ...
    conn.getSubtree({ oid: indexOid }, function (err, data) {
        function checkInterface(index) {
            var vb = data[index];
            if (!vb) {
                return;
            }
            var indexName = sanitize(vb.value);
...
function getUptime(conn, callback) {
    conn.get({ oid: [1, 3, 6, 1, 2, 1, 1, 3, 0] }, function (err, varbinds) {
        if (err) {
