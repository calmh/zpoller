...
function getPackage(conn, coll) {
    ...
    return q.when(psubtree(indexOid), function (data) {
        ...
        function checkInterface(index) {
            ...
            return getFilterResult(conn, indexId, coll.filter).then(function (res) {
                var oids, meta;
                if (res) {
...
function getUptimes(hosts) {
    var results = _.map(hosts, function (host) {
        var conn = new snmp.Session(host.ip, host.community);
        return q.when(getUptime(conn), function (u) {
            conn.close();
...
exports.mapHosts = function mapHosts(hosts, collect) {
    ...
    q.all(sessions).then(function () {
        ...
        log.warning('Indexing failed for some hosts.');
        deferred.resolve(_.values(result));
    });
